...
SubDiag = -1./dz_z' ;
SupDiag = 1./dz_z';
SupDiag = [SupDiag(1:2); zeros(Nz-2,1)] ; % just for forward derivative at bottom 
base    = spdiags( [SubDiag,Diag,SupDiag] , [-1,0,1] , Nz , Nz ) ;
DzBack  = kron( speye(Nn) , base ) ;

    % Forward approximation for value function
Diag        = 1./dz_z' ;
Diag(1)   = -1/dz_z(Nz) ; % approximate forward derivative with backward derivative at top
SupDiag     = 1./dz_z';
SupDiag     = [ SupDiag(1); zeros(Nz-1,1) ] ;
SubDiag     = -1./dz_z' ; % just for backward derivative at top
base        = spdiags( [SubDiag,Diag,SupDiag] , [-1,0,1] , Nz , Nz ) ;
